...
<!-- FRONTEND_HEADER_LOGO_ROADMAP_STAGE_START -->
## Обновление roadmap: header и логотип

Выполнено:

- добавлен базовый `Container`;
- добавлен модуль `Header`;
- логотип в `Header` ведёт на главную страницу;
- в `Header` подключён `ThemeSwitcher`;
- добавлена структура `src/modules` для крупных блоков приложения;
- добавлен компонент `Icon`;
- добавлен компонент `Logo`;
- добавлен кастомный SVG-логотип;
- подключён SVGR для импорта SVG как React-компонентов;
- SVG-иконка логотипа использует `currentColor`;
- добавлен отдельный шрифт для текста логотипа;
- добавлен отдельный цветовой token для текста логотипа.

Следующие задачи:

1. Финально подобрать SVG-форму логотипа.
2. Финально подобрать шрифт и цвет текста логотипа в light/dark темах.
3. Проверить `Header` на базовых ширинах экрана.
4. Подключить `Header` в общий `app/layout.tsx`.
5. После стабилизации `Header` перейти к настройке frontend env.
6. Добавить `NEXT_PUBLIC_API_URL`.
7. Создать API client.
8. Подключить первые backend-ручки:
   - `categories`;
   - `tags`;
   - `recipes`.
<!-- FRONTEND_HEADER_LOGO_ROADMAP_STAGE_END -->
